...
type TitlePage = { name: string, id: number, thumbnail: string };
type Template = { name: string, id: number, thumbnail: string };
...
export default function useDocumentCreationModal() {
  ...
  const [templatePages, setTemplatePages] = useState<Template[]>([]);
  useEffect(() => {
    fetch("/api/templates").then((r) => r.json()).then(r => { console.log(r); setTemplatePages(r as Template[]) }).catch(console.error)
  }, [isTemplateModalOpen])
      ...
      >
        <form className="min-w-sm" action={formAction}>
          ...
          <div className="w-full mt-4">
            ...
            <div className="flex w-full mt-2">
              <select className=" border border-slate-500/20 grow-1 bg-slate-100 px-4 py-2.5" name="template" id="template">
                <option value="undefined">&lt;Стандартный&gt;</option>
                {templatePages.map(t => (<option key={t.id} value={t.id.toString()}>{t.name}</option>))}
              </select>
